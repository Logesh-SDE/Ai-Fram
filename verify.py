#!/usr/bin/env python3
"""
AI-Farm Application Route Verification Script
This script checks if all routes are properly configured
"""
...
def check_routes():
    """Check if all required routes exist in app.py"""
    print("🔍 Checking AI-Farm Routes...\n")
    ...
    required_routes = [
        ...
        ('@app.route(\'/weather\')', 'Weather page'),
        ('@app.route(\'/disease-detection\')', 'Disease detection'),
        ('@app.route(\'/crop-recommendations\')', 'Crop recommendations'),
        ('@app.route(\'/fertilizer-calculator\')', 'Fertilizer calculator'),
        ('@app.route(\'/chatbot\')', 'AI Chatbot'),
        ('@app.route(\'/market-prices\')', 'Market prices'),
        ('@app.route(\'/govt-schemes\')', 'Government schemes'),
    ...
    try:
        ...
        for route, description in required_routes:
            if route in content:
                found_routes.append(description)
                print(f"✅ {description}")
            else:
        ...
        print("Make sure you're running this from the ai-farm directory")
        return False
...
def check_templates():
    ...
    print(f"\n📊 Summary:")
    print(f"✅ Found: {len(found)}/{len(required_templates)}")
...
def main():
    print("=" * 60)
    print("AI-Farm Application Verification")
    print("=" * 60)
    print()
    ...
    if routes_ok and templates_ok and deps_ok:
